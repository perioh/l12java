...
class Main {
    public static void main(String[] args) {
        Agency.Vacancy[] vacancies = {
                createVacancy("Company2", "IT", "Conditions1", "149200", "IT", "12", ""),
                createVacancy("Company1", "Waiter", "Conditions2", "69100", null, "1", "school")
        };

        Agency.Vacancy vacancy3 = createVacancy("Company3", "Teacher", "Conditions3", "19100", null, "24", "university");

        GenericContainer<Agency.Vacancy> generic = new GenericContainer<>();
        for (Agency.Vacancy vacancy : vacancies) {
            if (vacancy != null) {
                generic.pushBack(vacancy);
            }
        }
        generic.pushBack(vacancy3);

        System.out.println("\n-------\nINITIAL:");
        printVacancies(generic);

        System.out.println("\n-------\nSORT BY COMPANY NAME:");
        printVacancies(Utils.sortByCompanyName(generic));

        System.out.println("\n-------\nSORT BY EDUCATION REQUIRED:");
        printVacancies(Utils.sortByEducation(generic));

        System.out.println("\n-------\nSORT BY SPECIALIZATION:");
        printVacancies(Utils.sortBySpecialization(generic));
...
class Agency {
    ...
    public static class Vacancy implements Comparable<Vacancy> {
    ...
    public static class WorkerRequirements {
...
class GenericContainer<T> implements Iterable<T> {
